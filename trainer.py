...
import config
import data_manager
from mfg_engine import compute_mfg_scores
...
def main():
    ...
    for universe_name, tickers in config.UNIVERSES.items():
        print(f"\n=== Universe: {universe_name} (MFG Engine) ===")
        ...
        best_per_etf   = {}   # ticker → (score, window)
        window_results = {}   # window → {ticker: score}

        for win in config.WINDOWS:
            min_bars = win + config.VOL_WINDOW + 10
        ...
        if not best_per_etf:
            print("  No MFG scores — falling back to historical mean return")
        ...
        top_etfs    = [
            {"ticker": t, "mfg_score": float(s), "best_window": int(w)}
            for t, (s, w) in sorted_etfs[:config.TOP_N]
        ]

        print(f"  Top {config.TOP_N}: {[e['ticker'] for e in top_etfs]}")
        for e in top_etfs:
            print(f"    {e['ticker']}: {e['mfg_score']:.4f}  (window: {e['best_window']}d)")
    ...
    tab1_path = Path(f"results/mfg_engine_{today}.json")
    ...
    tab2_path = Path(f"results/mfg_engine_windows_{today}.json")
