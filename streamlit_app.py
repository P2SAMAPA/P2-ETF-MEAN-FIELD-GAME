...
st.set_page_config(page_title="MFG Nash Engine", layout="wide")

st.markdown("""
<style>
.main-header { font-size:2.4rem; font-weight:700; color:#1a3a5c; margin-bottom:0.3rem; }
.sub-header  { font-size:1.1rem; color:#555; margin-bottom:1.5rem; }
.uni-title   { font-size:1.4rem; font-weight:600; margin-top:1rem; margin-bottom:0.8rem;
               padding-left:0.5rem; border-left:5px solid #2471a3; }
.etf-card    { background:linear-gradient(135deg,#1a3a5c 0%,#2471a3 100%); color:white;
               border-radius:14px; padding:1rem; margin:0.4rem; text-align:center;
               box-shadow:0 4px 6px rgba(0,0,0,0.2); }
.win-card    { background:linear-gradient(135deg,#154360 0%,#1a5276 100%); color:white;
               border-radius:14px; padding:1rem; margin:0.4rem; text-align:center;
               box-shadow:0 4px 6px rgba(0,0,0,0.2); }
...
st.markdown('<div class="main-header">♟️ Mean Field Game Nash Engine</div>',
            unsafe_allow_html=True)
st.markdown(
    '<div class="sub-header">Lasry & Lions (2007) MFG framework · '
    'Nash equilibrium controls under crowding · '
    'Picard fixed-point solver · Multi-window cross-sectional z-score ranking</div>',
    unsafe_allow_html=True)

st.sidebar.markdown("## ♟️ MFG Nash Engine")
st.sidebar.markdown(f"**Next Trading Day:** `{next_trading_day()}`")
st.sidebar.markdown(f"**Windows:** {config.WINDOWS}")
st.sidebar.markdown(
    f"**MF Coupling κ:** {config.MF_COUPLING} | "
    f"**Risk Aversion γ:** {config.GAMMA_RISK}")
st.sidebar.markdown(
    f"**Weights:** Drift {config.WEIGHT_DRIFT:.0%} | "
    f"Crowding {config.WEIGHT_CROWDING:.0%} | "
    f"Macro {config.WEIGHT_MACRO:.0%}")
...
tab1_path = find_latest(files, "mfg_engine_2")
tab2_path = find_latest(files, "mfg_engine_windows_")
...
# ══════════════════════════════════════════════════════════════════════════════
with tab1:
    st.header("🏆 Top ETFs — MFG Nash Equilibrium Signal")

    with st.expander("📖 Mean Field Game Methodology", expanded=True):
        st.markdown("""
**Mean Field Game (MFG)** models ETF agents as rational optimisers who interact
through a shared *mean field* — the average market position — rather than
observing each other individually.

**Agent objective (agent i):**
```
...
| Term | Meaning |
|------|---------|
| `μᵢ·uᵢ` | Drift reward — profit from trading in direction of estimated return |
| `γ/2·σᵢ²·uᵢ²` | Risk penalty — CARA risk aversion weighted by variance |
| `λ·uᵢ²` | Execution cost — quadratic Almgren-Chriss market impact |
| `κ·m(t)·uᵢ` | **Crowding penalty** — mean field interaction term |

**Nash equilibrium condition:**
```
uᵢ*(t) = (μᵢ − κ·m*) / (γ·σᵢ² + 2λ)     m* = (1/N) Σ uᵢ*
```

Solved via Picard fixed-point iteration. Composite score = equilibrium drift
minus crowding penalty plus macro alignment, cross-sectionally z-scored.
        """)
...
<div class="etf-card">
  ...
  <div class="etf-score">MFG score = {etf['mfg_score']:.4f}</div>
        ...
        with st.expander(f"📋 Full ranking — {universe_name}"):
            ...
            if full:
                ...
                for t, info in full.items():
                    ...
                    rows.append({"ETF": t, "MFG Score": score, "Best Window (d)": win})
                df = pd.DataFrame(rows).sort_values("MFG Score", ascending=False)
                st.dataframe(df, use_container_width=True, hide_index=True)
        st.divider()

    st.caption(
        f"Run date: {data1.get('run_date','?')} · "
        "Lasry & Lions (2007) MFG framework · "
...
# ══════════════════════════════════════════════════════════════════════════════
with tab2:
    st.header("🔍 Explore MFG Rankings by Window")

    if not universes2:
        st.warning("Window-level detail not found. Re-run trainer to generate "
                   "`mfg_engine_windows_<date>.json`.")
        st.stop()
    ...
    with st.expander("ℹ️ Window guidance", expanded=False):
        st.markdown("""
- **63d** — short-term equilibrium; captures recent crowding and momentum dynamics
- **126d** — 6-month horizon; balances signal and noise
- **252d** — 1-year horizon; most stable Nash equilibrium estimates
- **504d** — 2-year horizon; structural crowding and macro regime alignment
        """)

    st.markdown(f"### MFG Rankings at **{selected_win}d** window")
...
""", unsafe_allow_html=True)
        ...
        with st.expander(f"📋 Full ranking — {label} @ {selected_win}d"):
            ...
            if rows:
                df = pd.DataFrame(rows, columns=["ETF", "MFG Score"])
